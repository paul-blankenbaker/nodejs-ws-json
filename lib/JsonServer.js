...
class JsonServer {
   ...
   * verifies itself, you should invoke the setAuthenticated(cc)
   * method.</p>
   *
  ...
  setAuthenticated(cc) {
    if (this.shouldLog(2)) {
      console.log("Granted access (authenticated) to client: " + cc.toString());
    }
  ...
  /**
   * Internal method responsible for initializing a new
   * ClientConnection when a new client connects.
   *
   * @param {Object} ws A ws.WebSocket connection used to communicate
   * with client.
   * @param {Object} req The http.IncomingMessage request received from client.
   */
  createClient(wsc, req) {
    const cc = new ClientConnection(this, wsc, req);
    wsc.cc = cc;
    if (this.shouldLog(1)) {
    ...
    if (this.authHandlers !== undefined) {
      ...
      cc.setHandlers(this.authHandlers);
    } else {
      // No "login" authentication required, enable all handlers
      this.setAuthenticated(cc);
    }
  }
